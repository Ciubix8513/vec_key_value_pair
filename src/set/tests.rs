...
#[test]
fn test_disjoint() {
    let mut xs = VecSet::new();
    let mut ys = VecSet::new();
...
fn test_from_iter() {
    ...
    let set: VecSet<_> = xs.iter().cloned().collect();
...
fn test_eq() {
    ...
    let mut s1 = VecSet::new();

    s1.insert(1);
    s1.insert(2);
    s1.insert(3);

    let mut s2 = VecSet::new();
...
#[test]
fn test_drain() {
    let mut s: VecSet<_> = (1..100).collect();
...
fn test_retain() {
    let xs = [1, 2, 3, 4, 5, 6];
    let mut set: VecSet<i32> = xs.iter().cloned().collect();
    set.retain(|&k| k % 2 == 0);
    assert_eq!(set.len(), 3);
...
#[test]
fn from_array() {
    let set = VecSet::from([1, 2, 3, 4]);
    let unordered_duplicates = VecSet::from([4, 1, 4, 3, 2]);
    ...
    // that's a problem!
    let _must_not_require_type_annotation = VecSet::from([1, 2]);
}
